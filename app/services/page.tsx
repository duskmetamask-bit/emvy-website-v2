...
const stages = [
  {
    number: '01', id: 'consult', title: 'AI Consult', imageSide: 'left',
    ...
    description: 'A focused, free 20-minute conversation about the part of the business you would like to make easier. We start with the real work, not a pre-set package.',
    points: ['Bring a question, frustrating process, or idea worth exploring.', 'Talk through what is happening now and what a better next step could look like.', 'Leave with a clearer sense of whether an AI Assessment or scoped build is useful.'],
  },
  {
    ...
    {stages.map((stage) => <section className="section service-stage" id={stage.id} key={stage.id}>
      <div className={`service-stage__grid ${stage.imageSide === 'right' ? 'service-stage__grid--reverse' : ''}`}>
        ...
        <div className="service-stage__copy">
          ...
          {stage.id === 'consult' ? <a href="https://cal.com/jake-emvy/discovery-call" className="inline-link" target="_blank" rel="noopener noreferrer">Book a 20-minute AI Consult</a> : null}
        </div>
      </div>
    </section>)}

    <section className="section final-cta-section">
      <div className="section-header"><p className="section-kicker">Start here</p><h2 className="section-title">Not sure which stage fits? Start with a conversation.</h2><p className="section-text">A free 20-minute AI Consult is a simple place to start. We can work out the useful next step together.</p></div>
